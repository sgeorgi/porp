...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers with a leading zero") {
    assert(sum(List(0, 2, 5)) === 7)
  }
  test("sum of a list with only one number") {
    assert(sum(List(2)) === 2)
  }
  ...
  test("max of a few numbers with repetition") {
